...
impl PeerAssignments {
    pub(crate) fn load<'a, P: AsRef<Path>>(path: P, peers: impl Iterator<Item=&'a PeerId>) -> Result<PeerAssignments, Error> {
        ...
        Ok(PeerAssignments{
            ...
            peers: peers.map(|p| p.clone()).collect(),
        })
    }
...
fn next_assignment<'a>(peers: &Vec<PeerId>, assignments: impl Iterator<Item=(&'a GithubUserId, &'a mut PeerId)>) -> PeerId {
    let assignment_counts: HashMap<PeerId, u64> = assignments.fold(HashMap::new(), |mut acc, (_, peer_id)| {
        acc.entry(peer_id.clone()).and_modify(|e| *e += 1).or_insert(1);
        acc
    });
