    ...
    pub(crate) fn load<'a, P: AsRef<std::path::Path>>(
        ...
        if std::fs::try_exists(&index_path)? {
            let key_by_peer: HashMap<PeerId, SecretKey> =
                peers.map(|(p, s)| (p.clone(), s.clone())).collect();
            ...
            for (peer, oid) in mapping {
                ...
                let key = key_by_peer
                    .get(&peer)
                    .ok_or_else(|| Error::MissingPeer { peer: peer.clone() })?;
            ...
            for (peer, key) in peers {
                ...
                ids.insert(peer.clone(), (identity, key.clone()));
            }
            let oid_mapping: HashMap<&PeerId, radicle_git_ext::Oid> = ids
                .iter()
                .map(|(p, (id, _))| (p, id.content_id.into()))
                .collect();
